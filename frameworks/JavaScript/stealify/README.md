# Fast Syntax Iteration
cd frameworks/JavaScript/stealify 
docker build -f stealify.dockerfile -t d . && docker run -it d
../../../tfb --mode verify --test stealify
# Congratulations!
...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `stealify.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `stealify-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `stealify-mysql`.
...
Edit `.travis.yml` to ensure that Travis-CI will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=JavaScript/stealify` should be inserted under `env > matrix` and put it there.
